...
class DashPeriod:
    def __init__(self, xml, parent):
        ...
        if base_url is not None:
            if IsAbsoluteURL(base_url.text):
                self.base_urls = [base_url.text]
            else:
                self.base_urls = [parent.base_urls[0] + base_url.text]
        self.segment_base = DashSegmentBaseInfo(xml)
...
class DashMPD:
    def __init__(self, url, xml):
        ...
        # compute base URL from MPD URL
        mpd_base = '/'.join(url.split('/')[:-1]) + '/'
        self.base_urls = [mpd_base]
        base_url = self.xml.find(DASH_NS+'BaseURL')
        if base_url is not None:
            if IsAbsoluteURL(base_url.text):
                self.base_urls = [base_url.text]
            else:
                self.base_urls = [mpd_base + base_url.text]
...
def OpenURL(url):
    if url.startswith("file://"):
        ...
        req.add_header('x-ad-insertion', 'true')  # Extra header for ComHem
...
def main():
    ...
    if mpd.type == 'dynamic':
        ...
        for i in range(1800):
            print "### Iteration %d" % i
            clone_asset(mpd_url, output_dir, i)
            this_time = time.time()
            if this_time - last_time < 2:
                time.sleep(2 - (this_time - last_time))
            last_time = this_time

    else:
        clone_asset(mpd_url, output_dir)
...
def clone_asset(mpd_url, output_dir, clone_iteration=-1):
    "Clone to output_dir using Options"
        ...
        for adaptation_set in period.adaptation_sets:
            for representation in adaptation_set.representations:
                ...
                if bandwidth > 1000000:
                    print "Skipping %s" % representation.id
                    continue
                ...
                cloner.Cleanup()
                print("Cloned %d of %d segment for %s" % (nr_segments_cloned,
                                                          nr_segments,
                                                          representation.id))
    ...
    output_name = os.path.join(output_dir,
                               os.path.basename(urlparse.urlparse(
                                   mpd_url).path))
    if clone_iteration >= 0:
        output_name += "_%04d" % clone_iteration
